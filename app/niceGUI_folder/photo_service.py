...
class PhotoService:
    ...
    @classmethod
    def create_photo_gallery(cls, photo_paths: List[str], max_width: str = "200px") -> ui.row:
        """Create photo gallery display"""
        gallery = ui.row().classes('flex-wrap gap-2')
        ...
        for photo_path in photo_paths:
            if photo_path and os.path.exists(photo_path):
                with gallery:
                    ...
                    if photo_url:
                        ui.image(photo_url).classes(f'max-w-{max_width} rounded shadow')
